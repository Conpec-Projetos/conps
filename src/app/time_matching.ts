...
async function saveSlotsToDB(slots: Slot[]) {
    try {
        // Save the previous matchings to "previous_matchings" collection
        const latestMatchingRef = collection(db, "latest_matching");
        ...
        if (!latestSnapshot.empty) {
            ...
            await setDoc(doc(previousMatchingRef, timestamp), { slots: previousSlots });
        ...
        await addDoc(latestMatchingRef, { slots: slotsData, timestamp: newTimestamp });
...
async function getInfo(): Promise<[Candidate[], Interviewer[], Slot[], number, number, number]> {
    const data = await getDocs(collection(db, "tests_1"));
    const info = data.docs[0].data();

    const candidates: Candidate[] = info.Candidates.map((person: any) => 
        new Candidate(person.Name, person.Gender, person.Availability)
    );
    candidates.sort((a, b) => a.possibleSlots - b.possibleSlots);

    const interviewers: Interviewer[] = info.Interviewers.map((person: any) =>
    ...
    for (const day in info.AvailableSlots.datetimes) {
        const times = info.AvailableSlots.datetimes[day as keyof typeof info.AvailableSlots.datetimes];
    ...
    return [candidates, interviewers, availableSlots, maxNumCandPerSlot, minNumCandPerSlot, numIntPerSlot];
}
...
function greedyMatching(
    ...
    const updateSwops = (slotsWithOnePerson: Map<Slot, boolean>): Map<Slot, boolean> => {
    ...
    const reassignmentIfNeeded = () => {
        if (maxNumCandPerSlot > 1) {
            ...
            while (slotsWithOnePerson.size > 0) {
                ...
                slotsWithOnePerson = updateSwops(slotsWithOnePerson);
                greedyMatchingMethodThree(slotsWithOnePerson, slotsWithMoreThanTwoPeople);
                slotsWithOnePerson = updateSwops(slotsWithOnePerson);
            }
        }
    };
    ...
    if (maxNumCandPerSlot > 1) {
        const slotsWithFewPeople = availableSlots.filter(
            slot => slot.candidates.length < minNumCandPerSlot && slot.candidates.length > 0
        );
    ...
    for (const slot of availableSlots) {
        if (slot.interviewers.length < numIntPerSlot && slot.candidates.length > 0) {
...
async function algorithm() {
    const [candidates, interviewers, availableSlots, maxNumCandPerSlot, minNumCandPerSlot, numIntPerSlot] = await getInfo();
    greedyMatching(candidates, maxNumCandPerSlot, minNumCandPerSlot, interviewers, numIntPerSlot, availableSlots);
    saveSlotsToDB(availableSlots);
